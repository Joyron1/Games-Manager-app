...
class Basketball extends Component {
    ...
    async getGamesByCategory() {
        console.log()
        try {
            await axios.get(`http://www.localhost:4000/getGamesByCat?category=Basketball`).then(response => {
            ...
            table = this.state.games.map(game => {
                return (
                    <tr>
                        ...
                        <td>{game.category}</td>
                        <td><Link to={"/basketball/" + game.id}> <button type="button" className="btn btn-secondary"> more info </button> </Link> </td>
                    </tr>
                )
            })
        }
